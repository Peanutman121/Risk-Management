...
class BBGDataProvider(DataProvider):
    ...
    def get_live_price(self, ticker: str) -> Optional[float]:
        ...
        try:
            ...
            # Note: flds should be a list for consistency with xbbg
            result = self.blp.bdp(tickers=ticker, flds=['PX_LAST'])

            if result is not None and not result.empty:
                # Column name might be 'px_last' or 'PX_LAST'
                col_name = 'px_last' if 'px_last' in result.columns else 'PX_LAST'
                if col_name in result.columns:
                    price = result.loc[ticker, col_name] if ticker in result.index else result.iloc[0][col_name]
                    return float(price) if not pd.isna(price) else None
                else:
                    logger.warning(f"Unexpected columns in BDP result: {result.columns.tolist()}")
                    return None
            else:
    ...
    def get_historical_data(
        ...
        try:
            ...
            # Note: flds must be a list for xbbg
            result = self.blp.bdh(
                tickers=ticker,
                flds=['PX_LAST'],
            ...
            if result is not None and not result.empty:
                ...
                if isinstance(df.columns, pd.MultiIndex):
                    # Flatten: take the ticker (first element of tuple)
                    df.columns = [col[0] for col in df.columns]
                
                # The column name is now the ticker, rename to 'price'
    ...
    def get_correlation_matrix(
        ...
        try:
            ...
            calendar_days = int(lookback * 1.4) + 10
            end_date = datetime.now()
            start_date = end_date - timedelta(days=calendar_days)
            ...
            if result is None or result.empty:
                logger.warning("No data returned for correlation matrix")
                return None
            
            # Flatten multi-index columns (take ticker name)
            df = result.copy()
            if isinstance(df.columns, pd.MultiIndex):
                df.columns = [col[0] for col in df.columns]
